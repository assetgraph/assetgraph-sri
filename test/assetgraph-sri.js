...
const assetGraphSri = require('../lib/assetgraph-sri');

describe('assetgraph-sri', function() {
  describe('in update mode', function() {
    it('should add integrity attributes', async function() {
      const assetGraph = new AssetGraph({
        root: pathModule.resolve(
          __dirname,
          '../testdata/noExistingIntegrityAttributes/'
        )
      });
    ...
    it('should leave existing integrity tokens alone', async function() {
      const assetGraph = new AssetGraph({
        root: pathModule.resolve(
          __dirname,
          '../testdata/existingIntegrityAttributes/'
        )
      });
      await assetGraph.loadAssets('index.html');
      await assetGraph.populate();
    ...
    describe('with single:true', function() {
      it('should warn and overwrite existing access tokens if they do not match', async function() {
        ...
        const assetGraph = new AssetGraph({
          root: pathModule.resolve(
            __dirname,
            '../testdata/wrongExistingIntegrityAttributes/'
          )
        });
        ...
        expect(warnSpy, 'to have calls satisfying', () => {
          warnSpy(
            new Error(
              'testdata/wrongExistingIntegrityAttributes/index.html: integrity hash does not match that of the linked resource (testdata/wrongExistingIntegrityAttributes/styles.css) sha512-/12B+GMz2RvTlokFsMOh9oKDW4+ETBrX8RmQkCB2J8bfsrXE3CKEY+sdpU4FngOMhyTfznYSZHNO7BOkF/0vmQ=='
            )
          );
          warnSpy(
            new Error(
              'testdata/wrongExistingIntegrityAttributes/index.html: integrity hash does not match that of the linked resource (testdata/wrongExistingIntegrityAttributes/script.js) sha512-wXdnLCdTwtIg7nRmZzmxKmpUFFLgOLafZ7VkIlTq4qzWaLtxCyGePw44M18vetUNA8RoVobBLwIpFCKAaRgJ1g=='
            )
          );
        });
      });
  ...
  describe('in validate mode', function() {
    it('should complain about invalid hashes', async function() {
      ...
      expect(warnSpy, 'to have calls satisfying', () => {
        warnSpy(
          new Error(
            'testdata/wrongExistingIntegrityAttributes/index.html: integrity attribute sha512-/12B+GMz2RvTlokFsMOh9oKDW4+ETBrX8RmQkCB2J8bfsrXE3CKEY+sdpU4FngOMhyTfznYSZHNO7BOkF/0vmQ== does not match the linked resource: testdata/wrongExistingIntegrityAttributes/styles.css'
          )
        );
        warnSpy(
          new Error(
            'testdata/wrongExistingIntegrityAttributes/index.html: integrity attribute sha512-wXdnLCdTwtIg7nRmZzmxKmpUFFLgOLafZ7VkIlTq4qzWaLtxCyGePw44M18vetUNA8RoVobBLwIpFCKAaRgJ1g== does not match the linked resource: testdata/wrongExistingIntegrityAttributes/script.js'
          )
        );
      });
    });
    ...
    it('should not complain when at least one hash is correct', async function() {
      const warnSpy = sinon.spy().named('warn');
      const assetGraph = new AssetGraph({
        root: pathModule.resolve(__dirname, '../testdata/multipleHashes/')
      });
      assetGraph.on('warn', warnSpy);
      await assetGraph.loadAssets('index.html');
      await assetGraph.populate();
